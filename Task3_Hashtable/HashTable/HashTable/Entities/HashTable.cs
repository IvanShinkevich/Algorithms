...
namespace HashTable.Entities
{
    ...
    public class HashTableChains
    {
        ...
        private readonly int maxArraySize = 1000;
        ...
        /// </remarks>
        private Dictionary<float, List<Item>> _items = null;

        /// <summary>
        /// Коллекция хранимых данных в хеш-таблице в виде пар Хеш-Значения.
        /// </summary>
        public IReadOnlyCollection<KeyValuePair<float, List<Item>>> Items => _items?.ToList()?.AsReadOnly();
        ...
        public HashTableChains()
        {
            ...
            _items = new Dictionary<float, List<Item>>(_maxSize);
        }
        ...
        private float GetHash(string value)
        {
            // Проверяем входные данные на корректность.
            if (string.IsNullOrEmpty(value))
            {
